...
async function reservationIsValid(req, res, next) {
  ...
  if (!reservation_id)
    return next({
      status: 400,
      message: "reservation_id must be included in the body of the request",
    });
  ...
  if (reservation) {
    if (reservation.people > res.locals.table.capacity)
      return next({
        status: 400,
        message: "reservation party size exceeds capacity",
      });

    if (reservation.status === "seated")
      return next({
        status: 400,
        message: "reservation party is already seated",
      });
...
function isValidTable(req, res, next) {
  ...
  if (!capacity)
    next({ status: 400, message: `capacity must exist and have a value` });
