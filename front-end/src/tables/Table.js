...
function Table({ table, loadPage}){
    ...
    return (
        <div className="d-flex text-center">
            ...
            {status==="free"? 
            <div className="col mb-0 alert alert-success" data-table-id-status={table_id}>{status}</div>
            :<div className="col mb-0 alert alert-danger" data-table-id-status={table_id}>{status}</div>}
            {status === "occupied" ? 
            <button 
